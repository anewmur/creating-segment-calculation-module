...
def test_merge_by_radius_skips_when_same_neighbor_has_two_vertices_in_radius():
    ...
    assert infos == [
        'Расчёт сегментов\n'
        'Для полилиний полигона Полигон с индексами 0 в радиус склейки входит '
        'более 1 точки одной полилинии. Склейка для них не выполнена.',
    ]
...
def test_merge_by_radius_excludes_polygon_after_self_intersection():
    ...
    assert warnings == [
        'Расчёт сегментов\n'
        'Полилиния полигона Полигон с индексом 0 исключена из расчёта из-за '
        'самопересечения после склейки.',
    ]
